...
sos.base = {
    ...
    exInSidePop : function () {
        ...
        if ($('.s-exin').length) {
            ...
            var $pop = $('.popSideBox');
            var handlerCls = 's-ex';
            var maskCls = 'so-showmask';
            var popWith = '14em';
            $handler.on('tap',function () {//handler点击时收起或展开
                if($(this).hasClass(handlerCls)){
                    me.inSidePop($handler,handlerCls,$mask,maskCls,$pop,popWith);//收起sidepop
                }else{
                    me.exSidePop($handler,handlerCls,$mask,maskCls,$pop);//展开sidepop
                }
            });
            $pop.on('swipeRight',function () {//pop向右滑动，收起
                 me.inSidePop($handler,handlerCls,$mask,maskCls,$pop,popWith);
            });
        };

    },
    exSidePop : function ($handler,handlerCls,$mask,maskCls,$pop) {
        $handler.addClass(handlerCls);
        $mask.addClass(maskCls);
        $pop.css({
            right : '1px'
        });
    },
    inSidePop : function ($handler,handlerCls,$mask,maskCls,$pop,popWith) {
        $handler.removeClass(handlerCls);
        $mask.removeClass(maskCls);
        $pop.css({
            right : -popWith*1
        });
    },
