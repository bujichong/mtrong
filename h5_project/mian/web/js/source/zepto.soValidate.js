		...
		var vv = {
			validate : function (opt) {//主验证函数
				...
				if (o.validate) {
					...
					$validateBtn.on('touchend',function (e) {//开启验证
						e.preventDefault();
			...
			addArea : function (area) {//增加area范围内的inputs验证
				$inputs.unbind('blur.validate');
				var $add = $(area).find('input,select,textarea');
				$inputs = $inputs.add($add);
				$inputs.bind('blur.validate',function () {
					vv._blurValidate(this);
				});
				return $inputs;
			...
			clearArea : function (area) {//移除area范围内的inputs验证，并清除验证样式
				$inputs.unbind('blur.validate');
				var $not= $(area).find('input,select,textarea');
				$inputs = $inputs.not($not);
				$not.removeClass(o.errorCls);
