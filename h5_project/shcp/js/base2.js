...
$.touchSlider = function(opt) {
    ...
    $(o.thumb).removeClass(o.nowCls).eq(o.startSlide).addClass(o.nowCls);
    var slider = Swipe(document.getElementById(o.galleryWrapId), {
        auto: o.auto,
        startSlide : o.startSlide,
        continuous: true,
    ...
    window.console && console.log(slider);
    $(o.thumb).on('click',function() {
        var indx = $(o.thumb).index(this);
        slider.slide(indx);
        return false;
    });
};
...
menu:function()
{
     ...
     $tc.on('click',function() {
         ...
         $tc.removeClass("li-now");
         $(this).addClass("li-now");
    });
    $tc_li.on('click',function() {
         $tc.removeClass("li-now");

        $(this).parent(".li_nav").addClass("li-now");
    });
...
$(".pay1").on('click',function() {
    ...
    $(".pay2").removeClass("s-exSel");
    $(".pay1").addClass("s-exSel");
    $("#payment").val("1");
    $(".em-express").html("货到付款");
}
);

$(".pay2").on('click',function() {
    $(".pay1").removeClass("s-exSel");
    $(".pay2").addClass("s-exSel");
      $("#payment").val("2");
       $(".em-express").html("现货现付");
}
);
},
    ...
    extendSearch: function() { //主导航点击展开搜索
        if ($('.s-exSearch').length) {
            $('.s-exSearch').on('click',function() {
                $('.searchbox').toggleClass('searchShow');
                moveChk();
            });
            $('.s-extend').on('click',function() {
    ...
    subKindsEx: function() { //二级分类点击展开
        if ($('.li-subKinds').length) {
            $('.li-subKinds').find('.s-title').on('click',function() {
        ...
        if ($('.fn-details').length) {
            ...
            $('.fn-details').on('click',function() { //查看详情
               ...
               // alert($is_collect);
                ...
                // if($is_collect>0)
                // {
                //     $del="<a href='javascript:del_collect("+data.id+")'><span class='s-zp' >从常购商品中删除</span></a>";
                //     $('.center').html($del);
                // };
                // if($is_collect == 0)
                // {
                //     $del=" <span class='s-op'>从常购商品中删除</span>"
                //     $('.center').html($del);
                // };
                me.buyPop   =  $.pop({
                    target: '.proDetail',
          ...
          if ($('.fn1-details').length) {
             var me=this;
            $('.fn1-details').on('click',function() { //查看详情
                var data = $(this).parents('.li-list').data('details');
                $is_collect=$(this).parents('.li-list').attr('is_collect');
               // alert($is_collect);
                //window.console && console.log(data);
                ...
                // if($is_collect>0)
                // {
                //     $del="<a href='javascript:del_collect_2("+data.id+")'><span class='s-zp' >从常购商品中删除</span></a>";
                //     $('.center').html($del);
                // };
                // if($is_collect == 0)
                // {
                ...
                // };
                   ...
                   beforePop: function() {
                      ...
                      var sw = wh>ww?ww*0.85:wh*0.65;
                        ...
                        if ($is_collect > 0) {
                            $('.btn-setCollect').text('不常用').attr('href','javascript:del_collect_2('+data.id+')').bind('click',function () {
                                sos.base.buyPop.removePop();
                                $.loadTip({
                                    content : '删除成功！',
                                    removeDelay : 800//1.5s,默认1s
    ...
    listEventBuy: function() { //产品列表点击弹窗采购信息
        ...
        function beforePop($id) {
            ...
            var htm_price=num*parseFloat($price).toFixed(2);
                $('.price_'+$id).html("￥"+htm_price);
    ...
    numAd: function() {
       ...
       if ($('.fn-op-num').length) {
        ...
        if ($('.fn-op-num').length) {
            var $fn = $('.fn-op-num');
            $fn.find('.em-op-f').on('click',function() {//减少
        ...
        if($status == "checked")
     {
         ...
         $(".em-totalPrice").text("￥"+$new_amount);
            ...
            $fn.find('.em-op-e').on('click',function() {//增加
    ...
    cartChkAll: function() {//购物车选择商品事件
        if ($('.chkall').length) {
            var $t = $('.s-tipSel'),
                ...
                $one = $('.chkone'),
                $all = $('.chkall');
                $allamount=parseFloat($('.all_mount').val()).toFixed(2);
