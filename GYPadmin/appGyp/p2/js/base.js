...
$.views&&$.views.converters({//jsrender获取时间参数
    ...
    taskS : function (state) {
       ...
       (state===1)&&(txt='<span class="s-new s-follow">已接单</span>');
        (state===2)&&(txt='<span class="s-new s-finish">已完成</span>');
        return txt;
    }
});
...
var vv = {
    ...
    repairResult : function (form) {
        window.console && console.log(form.serializeObject());
        $.loadTip({content : '保存成功!',widthPer:0.6});
    },
    repairLog : function (form) {
        window.console && console.log(form.serializeObject());
        $.loadTip({content : '保存成功!',widthPer:0.6});
    }
};
...
sos.base = {
    init: function() {
        var me = this;
        me.formValidate();
         if ($('.dl-newMessage').length) {
            me.doMessage($('.dl-newMessage'));//默认newMessage处理事件
        }
    ...
    formV : null,
    doMessage : function ($dl) {
        $dl.find('.a-yes').click(function () {
            // $.confirm({
            //     content : '你确定接此单？',
            //     sure : function () {
            //         $.loadTip({content : '已接单成功！',widthPer : 0.6});
            //     }
            // });
            // return false;

                var repairTaskId = $(this).attr('rel');
                $.confirm({
                    content : '你确定接此单？',
                    sure : function () {
                        jQuery.ajax({
                                url: "/updateRepairTaskStatus.html",   // 提交的页面
                                data: {repairTaskId:repairTaskId},
                                type: "POST", // 设置请求类型为"POST"，默认为"GET"
                                success: function(data) {
                                    var data = eval(data);
                                    if(data.success){
                                        window.location.href="/repairList.html"; // 设置表单提交完成使用方法
                                    }else{
                                        $.loadTip({content : data.message,widthPer : 0.6});
                                    }
                                }
                            });
                        $.loadTip({content : '已接单成功！',widthPer : 0.6});
    ...
    loadMore : function () {//加载更多事件
        var me = this;
        $.get(loadOpt.url,{//第几页参数名pi ,每页多少条参数名：ps
            pi :me.pageNow,
            ps : loadOpt.cellLen
        },function (data) {
            if (data.data.length) {
                ...
                var $dls = $c.find('.dl-newMessage');
                if($dls.length){me.doMessage($dls);}//newMessage页面点击事件
            };
        })
    },
           ...
           me.formV =  $('.form-validate').soValidate({//验证登录弹窗里的表单
                ...
                submit : function (form) {//默认验证成功提交submit事件
                   // form.submit();//在此填写提交事件
                    ...
                    if (rel) {
                        vv[rel](form);//执行vv里对应rel的函数
                        window.console && console.log('执行vv里的 '+rel+'函数事件！请对应书写~');
                    };
                },
