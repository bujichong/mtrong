...
;(function () {
	...
	$.fn.soValidate = function (o) {
		...
		var vv = {
			...
			_byTip : function(obj,msg,msgCls,inputPar,msgPos) {//验证提示方法
				var $p = msgPos?$(msgPos):($(obj).parents(inputPar)?$(obj).parents(inputPar):$(obj).parent());//
				if (msg) {//错误状态
					if ($p.find('.'+msgCls+'').length<1) {
						$p.append('<em class="'+msgCls+'">'+msg+'</em>');
					}
					//if(type=="easydrop"){window.console && console.log($p,msgCls,msg,$p.find('.'+msgCls+''));}
	...
	$.soValidate.addRex({
		...
		easydrop : {
			rule : function (val,param,$o) {// param : number
				var v = $o.siblings('.combo').find('.combo-value').val();
				v = $.trim(v);
				return v===''?false:true;
			},
			msg:function (val,param) {
