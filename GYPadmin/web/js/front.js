...
$(function () {
	if ($('.form-validate').length) {
		$('.loginCont .txt').textFocus().focus(function () {
			if($(this).val()==''){
		...
		var ypuser = $.cookie('ypuser');
		var allowed = $('.chk-rememberUser:checked').length;
		if (ypuser&&$('.form-login').length) {
			$('.txt-user').val(ypuser).siblings('.lab-df').hide();
			//window.console && console.log(ypuser);
		};
		$('.chk-rememberUser').click(function () {//是否cookie用户名
			if ($('.chk-rememberUser:checked').length) {
				allowed = true;
			}else{
				allowed = false;
				//$.cookie('ypuser','');
			};
			//window.console && console.log(allowed);
		});
