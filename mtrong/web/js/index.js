...
mTrong.index = {
	...
	cartManyE : function () {
		...
		$('.minicart .txt-num').keyup(function () {//增加
			var num = $(this).val();
			num = num.replace(/\D/g,'');
	...
	bannerE : function () {
		$('.p-banner').soChange({
			thumbObj : '.h3-banner',
			thumbNowClass : 'h3-banner-now'
		});
	},
	tabE : function () {
		var $cont = $('.tabCont');
		$cont.soChange({
			thumbObj : '.h3-tabTitle',
			thumbNowClass : 'h3-tabTitle-now',
			autoChange : false,
			slideTime : 0,
			callback : function (prev,now) {
				var $img = $cont.eq(now).find('img');
				$img.each(function () {
					var src = $(this).attr('src4');
					 src&&$(this).attr('src',src).removeAttr('src4');
				});
			}
		});
	},
	...
	ajaxUrlSa : function (url,data,successback) {
		var t = this;
		$.ajax({
			url : url,
			data : JSON.stringify(data),
